...
<#
    .SYNOPSIS
        Makes a request to the installed Video Server to create a New Server Site
    .PARAMETER ServerHostName
        Optional Host Name or IP of the Machine on which the Video Server has been installed and is to be configured.
    .PARAMETER Ensure
        Ensure makes sure that a Video Server site is configured. Take the values Present or Absent. 
        - "Present" ensures that a Video server site is created.
        - "Absent" ensures that existing Video server site is deleted.
    ...
    .PARAMETER ServerDirectoriesRootLocation
        Path to Video Server Root Directories - Can be a Physical Location or Network Share Address
    ...
    .PARAMETER LogLevel
        Defines the Logging Level of Video Server. Can have values - "OFF","SEVERE","WARNING","INFO","FINE","VERBOSE","DEBUG" 
#>
...
function Set-TargetResource
{
    ...
    if($Ensure -ieq 'Present') {       
        ...
        if($RestartRequired) {
			...
			Write-Verbose "Waiting for Server 'https://$($FQDN):21443/arcgis/admin' to initialize"
            Wait-ForUrl "https://$($FQDN):21443/arcgis/admin" -HttpMethod 'GET' -Verbose
        }

        $ServerUrl = "https://$($FQDN):21443"
        Write-Verbose "Checking for Video Server site on '$ServerUrl'"
        $siteExists = $false
        try {  
	...
	while ((-not $Done) -and ($NumAttempts++ -lt 3)){                 
        ...
        if($response) {
			if ($response -and $response.status -and ($response.status -ieq "success")) {
				$Done    = $true
                $Success = $true
                Write-Verbose "Join Site operation successful. Waiting for $($response.pollAfter) seconds for Video Server to initialize."
