        ...
        for ( $i = 0; $i -lt $NodeRoleArray.Count; $i++ )
        {
            ...
            Switch($NodeRole)
            {
                            ...
                            if($Federation -and ($Node.NodeName -ieq $PrimaryServerMachine))
                            {
                                if(($AllNodes | Where-Object { $_.Role -icontains 'PortalWebAdaptor'}  | Measure-Object).Count -eq 0)
                                {
                                    if($PrimaryPortalMachine)
                                    {
                                        ...
                                        {
                                            $PrimaryPortalMachineNode = ($AllNodes | Where-Object { ($_.Role -icontains 'Portal' -and $_.NodeName -ieq $PrimaryPortalMachine)} | Select-Object -First 1)
                                            
                                            $PortalHostName = if($PrimaryPortalMachineNode.SslCertifcate.Alias){ $PrimaryPortalMachineNode.SslCertifcate.Alias }else{ Get-FQDN $PrimaryPortalMachine }
                                            $PortalPort = 7443
                                        ...
                                        throw('No Portal Details for Federation Given!')
                                    }
                                }
                                elseif(($AllNodes | Where-Object { $_.Role -icontains 'PortalWebAdaptor' }  | Measure-Object).Count -gt 0)
                                {
                                    if(($AllNodes | Where-Object { ($_.Role -icontains 'ServerWebAdaptor')}  | Measure-Object).Count -eq 0)
                                    {
                                        ...
                                        $PortalHostName = if($PortalWAMachineName.SslCertifcate.Alias){ $PortalWAMachineNode.SslCertifcate.Alias }else{ (Get-FQDN $PortalWAMachineName) }
                                        $PortalPort = 443
                                ...
                                if($PortalHostName -and $PortalPort -and $PortalContext )
                                {
                                    ...
                                    if($FederationFlag)
                                    {
                                        $ServerFedHostName = if($Node.SslCertifcate.Alias){ $Node.SslCertifcate.Alias }else{ $MachineFQDN }
                ...
                'Portal'{
                    ...
                    $ExternalDNSName = if($Node.SslCertifcate){ $Node.SslCertifcate.Alias}else{ (Get-FQDN $PrimaryPortalMachine.NodeName) }
                    ...
                    if(($AllNodes | Where-Object { $_.Role -icontains 'LoadBalancer' } | Measure-Object).Count -gt 0){
                        ...
                        $ExternalDNSName = if($LBMachine.SslCertifcate) { $LBMachine.SslCertifcate.Alias }else{ (Get-FQDN $LBMachine.NodeName) }
                    }else{
                        if((($AllNodes | Where-Object { ($_.Role -icontains 'PortalWebAdaptor')}  | Measure-Object).Count -gt 0) -and $ConfigurationData.ConfigData.PortalContext)
                        {
                            ...
                            $ExternalDNSName = if($PortalWAMachine.SslCertifcate) { $LBMachine.SslCertifcate.Alias }else{ (Get-FQDN $PortalWAMachine.NodeName) }
                        }
                    }
                    ...
                    if($Node.NodeName -ieq $PrimaryPortalMachine -and (($AllNodes | Where-Object { ($_.Role -icontains 'LoadBalancer')}  | Measure-Object).Count -eq 0))
                    {
                        if($Federation -and (($AllNodes | Where-Object { ($_.Role -icontains 'ServerWebAdaptor') -or ($_.Role -icontains 'PortalWebAdaptor')}  | Measure-Object).Count -eq 0))
                        {
                            ...
                            {
                                $PortalHostName = if($Node.SslCertifcate.Path){ $Node.SslCertifcate.Alias }else{ Get-FQDN $PrimaryPortalMachine }
                                $PortalPort = 7443
                                $PortalContext = 'arcgis'
                                $FederationFlag = $True
                                if($HostingServer)
                                ...
                                if($FederationFlag)
                                {
                                    $PrimaryServerMachineNode = ($AllNodes | Where-Object { ($_.Role -icontains 'Server' -and $_.NodeName -ieq $PrimaryServerMachine)} | Select-Object -First 1)
                                    $ServerFEDHostName = if($PrimaryServerMachineNode.SslCertifcate.Alias) { $PrimaryServerMachineNode.SslCertifcate.Alias }else{ (Get-FQDN $PrimaryServerMachine) }
                            ...
                            if($Federation -and (($AllNodes | Where-Object { $_.Role -icontains 'LoadBalancer' }  | Measure-Object).Count -eq 0) -and $Node.Role -icontains "ServerWebAdaptor")
                            {
                                if(($AllNodes | Where-Object { $_.Role -icontains 'PortalWebAdaptor' }  | Measure-Object).Count -gt 0)
                                {
                                    if(($AllNodes | Where-Object { ($_.Role -icontains 'PortalWebAdaptor') -and -not($_.Role -icontains 'ServerWebAdaptor')}  | Measure-Object).Count -gt 0)
                                    {
                                        ...
                                        $PortalHostName = if($PortalWAMachineNode.SslCertifcate){ $PortalWAMachineNode.SslCertifcate.Alias }else{ Get-FQDN $PortalWAMachineName }
                                    }
                                    else
                                    {
                                        $PortalHostName = if($Node.SslCertifcate){ $Node.SslCertifcate.Alias }else{ $MachineFQDN }
                                    }
                                    $PortalPort = 443
                                    $PortalContext = $ConfigurationData.ConfigData.PortalContext
                                }
                                else
                                {
                                    ...
                                    {
                                        $PrimaryPortalMachineNode = ($AllNodes | Where-Object { ($_.Role -icontains 'Portal' -and $_.NodeName -ieq $PrimaryPortalMachine)} | Select-Object -First 1)
                                        $PortalHostName = if($PrimaryPortalMachineNode.SslCertifcate){ $PrimaryPortalMachineNode.SslCertifcate.Alias }else{ Get-FQDN $PrimaryPortalMachine }
                                        $PortalPort = 7443
                                        $PortalContext = "arcgis"
                                        if($Node.WMFVersion -gt 4){
                                            WaitForAll "WaitForAllPortalConfigToCompleteServerFederation$($PrimaryPortalMachine)"{
                                    ...
                                    if($PortalHostName -and $PortalPort -and $PortalContext)
                                    {
                                        ...
                                        if($FederationFlag)
                                        {
                                            ...
                                            if($ConfigurationData.ConfigData.WebAdaptor.AdminAccessEnabled){
                                                ...
                                                $PrimaryServerMachineNode = ($AllNodes | Where-Object { ($_.Role -icontains 'Server' -and $_.NodeName -ieq $PrimaryServerMachine)} | Select-Object -First 1)
                                                if($PrimaryServerMachineNode.SslCertifcate.Alias){
                ...
                'LoadBalancer'{
                    ...
                    if(($AllNodes | Where-Object { $_.Role -icontains 'ServerWebAdaptor' -or $_.Role -icontains 'PortalWebAdaptor'}).count -gt 0)
                    {
                        ...
                        ForEach($svr in ($AllNodes | Where-Object { $_.Role -icontains 'ServerWebAdaptor' -or $_.Role -icontains 'PortalWebAdaptor'}))
                        {
                            ...
                            {
                                HostName = if($svr.SslCertifcate.Alias){ $svr.SslCertifcate.Alias }else{ Get-FQDN $svr.NodeName }
                                Ensure = 'Present'
                                ApplicationPath = "/"
                                HttpsPort = 443
                        ...
                        {    
                            if($MemberPortals)
                            {
                                $PortalHostName = if($Node.SslCertifcate.Alias){ $Node.SslCertifcate.Alias }else{ Get-FQDN $MachineFQDN }
                                $PortalPort = 443
                                $PortalContext = $ConfigurationData.ConfigData.PortalContext
                            }
                            else
                            ...
                            if($MemberServers)
                            {
                                $ServerHostName = if($Node.SslCertifcate.Alias){ $Node.SslCertifcate.Alias }else{ Get-FQDN $MachineFQDN }
                            ...
                            {
                                $PrimaryServerMachineNode = ($AllNodes | Where-Object { ($_.Role -icontains 'Server' -and $_.NodeName -ieq $PrimaryServerMachine)} | Select-Object -First 1)
                                $ServerHostName = if($PrimaryServerMachineNode.SslCertifcate.Alias) { $PrimaryServerMachineNode.SslCertifcate.Alias }else{ $PrimaryServerMachine  }
                            ...
                            if($PortalHostName -and $PortalPort -and $PortalContext -and $ServerHostName -and $ServerContext -and $ServerPort)
                            {
                                ...
                                if($FederationFlag)
                                {
                                    if($ConfigurationData.ConfigData.WebAdaptor.AdminAccessEnabled){
                                        ...
                                        $PrimaryServerMachineNode = ($AllNodes | Where-Object { ($_.Role -icontains 'Server' -and $_.NodeName -ieq $PrimaryServerMachine)} | Select-Object -First 1)
                                        $ServerSiteAdminUrlHostName =  if($PrimaryServerMachineNode.SslCertifcate.Alias){ $PrimaryServerMachineNode.SslCertifcate.Alias }else{ Get-FQDN $PrimaryServerMachine }
                                        $ServerSiteAdminUrlPort = 6443
                                        $ServerSiteAdminUrlContext = 'arcgis'
                                    }
